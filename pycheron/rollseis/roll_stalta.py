...
def roll_stalta(x, n_sta, n_lta, increment=1, fortran=False, logger=None):
    ...
    Simple rolling STA/LTA ratio calculation utilized for automatic detection of seismic signal arrival times.
    ...
    :return: Returns vector of values of the same length as x with each point containing the STA/LTA
                             ratio at that point
    :rtype: numpy.array

    .. note:: Values within n_lta - 1 of the beginning and n_sta - 1 of the end are set to NaNs
    ...
    **Plotting**
    ...
    if fortran:
        ...
        # For valid region, calculate the result
...
def stalta_python(x, n_sta, n_lta, ind):
    ...
    This is a simple ratio of two rolling means that is used to detect first breaks.
    ...
    :return: (vector) - out - Returns a vector of the same length as the incoming data with NaNs in the LTA window
                              length at the left end and in the STA window length at the right end
