...
def test_n_sta_gt_len_roll_stalta(stalta_assets):
    ...
    stalta = roll_stalta(stalta_assets["x"], n_sta, stalta_assets["n_lta"], stalta_assets["increment"])
    assert stalta is None


def test_n_lta_gt_len_roll_stalta(stalta_assets):
    n_lta = len(stalta_assets["x"]) + 1
    stalta = roll_stalta(stalta_assets["x"], stalta_assets["n_sta"], n_lta, stalta_assets["increment"])
    assert stalta is None


@pytest.mark.parametrize("increment", list(range(-10, 1)))
def test_increment_lt_1_roll_stalta(stalta_assets, increment):
    stalta = roll_stalta(stalta_assets["x"], stalta_assets["n_sta"], stalta_assets["n_lta"], increment)
    assert stalta is None
...
def test_valid_roll_stalta(stalta_assets):
    ...
    grab_expected_reals = [isinstance(x, np.float64) for x in stalta[stalta_assets["n_lta"] : -stalta_assets["n_sta"]]]
