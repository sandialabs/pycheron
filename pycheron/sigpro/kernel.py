...
def modified_daniell_kernel(m, logger=None):
    ...
    The modified daniell kernel halves the end coefficients.  Bloomfield [#]_
    ...
    :param m: "Kernel dimension(s) (e.g., number of weights or span of the filter). When m has a length larger
              than one, it means the convolution of kernels of dimensions ``m[j] for j in 1:length(m)``." [3]_ [#]_
    :type m: int
    ...
    * Code originally ported from R's kernel function
      https://rdrr.io/r/stats/kernel.html, which is part of R's
     ...
     .. note::
            ``kernel.R`` has more kernel options (dirichlet, fejar, daniell),
    ...
    **Example**
    ...
    .. [#] https://rdrr.io/r/stats/kernel.html
...
def _tskernel(k, m):
    ...
    * Code originally ported from R's kernel function
    ...
    # Initialize list to fill coefficients of daniel smoothing kernel
    y = []
...
def _kernapply(x, k, m, logger=None):
    ...
    * Code originally ported from R's kernel function
      https://rdrr.io/r/stats/kernapply.html, which is part of R's
      stats package.  R Core Team (2013). R: A language and environment for statistical computing. R
      Foundation for Statistical Computing, Vienna, Austria. ISBN 3-900051-07-0, URL
      http://www.R-project.org/. It has been augmented and adapted for use within Pycheron.
    ...
    .. note::
            Took out np.real calculation (originally had this). R code applies real restriction if y is numeric,
            though when circular is used only returns y. As not using non-circular option, simply omitted circular
            logical input argument. Enforcing y to be real is handled in crossSpectrum.py function.
