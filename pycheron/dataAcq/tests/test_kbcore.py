...
def test_read_kb_core_site(site_and_sitechan_assets, site_colnames):
    valid_df = kbcore.read_kb_core(
        site_and_sitechan_assets["dir"] + site_and_sitechan_assets["site"], "site"
    )
...
def test_prepare_for_site_or_sitechan(site_colnames, sitechan_colnames):
    ...
    valid_sitechan_cols, valid_sitechan_fun = kbcore._prepare_for_site_or_sitechan(
        "sitechan"
    )
    ...
    with pytest.raises(ValueError):
        invalid_cols, invalid_fun = kbcore._prepare_for_site_or_sitechan(
            "random string"
        )
