...
def deadChanADFMetric(
    ...
    :param win_size: window size to increment the data by in samples, each window will have a ADF run on it (assumes 100sps). If None, window size determined by sample rate of each trace in the stream object
    :type win_size: int
    :param pvalue_threshold: p value threshold to trigger ADF rejection of null hypothesis (e.g., time series is assumed stationary with rejection of null hypothesis)
    :type pvalue_thresh: float
    :param threshold: threshold to trigger ADF rejection of null hypothesis using difference between DF test statistic and 1% critical value
    ...
    :param use_thresh: String option for choosing whether to use the pvalue_threshold or the threshold for determining whether null hypothesis fails. Options are `pvalue` or `diffs`.
    :type use_thresh: str
    ...
    for s in range(len(st)):
        ...
        if inds.size != 0:
            ...
            for c in range(len(cons)):
                # Skip filter flukes and only process consecutive elements greater than con_thresh due to increased Type 1
                # errors in ADF test
                if int(tr.stats.sampling_rate) == 100:
                    # window filtering threshold is ~2.1 min to eliminate flukes
                    # potential TODO: 10 min is 120 for win_size 500, 100sps might be more effective to window out issues
                ...
                if len(cons[c]) >= con_thresh:
                    start = (
                        tr.stats.starttime
                        + (cons[c][0] * win_size) * 1.0 / tr.stats.sampling_rate
                    )
                    end = (
                        tr.stats.starttime
                        + (cons[c][-1] * win_size) * 1.0 / tr.stats.sampling_rate
                    )
            ...
            if generateMasks:
                ...
                if masksByTime:
                    ...
                    deadMask = []
                    # Loop through consecutive element blocks, skip single element issues. Append Starttime and Endtime to
                    # list, then convert to array
                    for c in range(len(cons)):
                        if int(tr.stats.sampling_rate) == 100:
                            # window filtering threshold is ~2.1 min to eliminate flukes
                            # potential TODO: 10 min is 120 for win_size 500, 100sps might be more effective to window out issues
                        ...
                        if len(cons[c]) >= con_thresh:
                            ...
                            deadMask.append(
                                {"Startime": s.isoformat(), "Endtime": e.isoformat()}
                            )
