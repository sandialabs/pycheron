...
def deadChannelMetric(st, logger=None, database_config=None):
    ...
    **Examples**
    ...
    # Initialize all list to append data to
    all = []

    # Loop through stream object and test if traces are dead
    for i in range(len(st)):
        # Get trace
        tr = st[i]
        # Test if it is flat-lined, demean=True, detrend=False, 0 taper applied
        dc = isDC(tr, False, True, 0)
        # For each trace create dictionary of metric information
        data = {
            "is_dead_channel": dc,
            "snclq": tr.get_id(),
        ...
        # Append trace metric information to all list, then keep looping through traces
        all.append(data)
