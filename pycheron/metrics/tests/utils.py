...
def percent_diff(expected, evaluated, diff):
    return (expected == 0.0 and evaluated == 0.0) or (abs(expected - evaluated) / expected) < diff
...
def make_dict_of_lists(list_of_dicts):
    return {key: [d[key] for d in list_of_dicts] for key, v in list(list_of_dicts[0].items())}
