...
def repeatedAmplitudeMetric(
    st, minRep=10, generateMasks=False, logger=None, fortran=False, database=None
):
    """
...
def _repeatedAmplitudeMetric(
    tr, minRep=10, generateMasks=False, logger=None, fortran=False
):
    """
    ...
    if not fortran:
        vals = []
        index = []
        # goes thru data checks from data point i to i + minRep are the same.
