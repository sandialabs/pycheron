...
def repeatedAmplitudeMetric(
    tr, minRep=10, bound=10, generateMasks=False, logger=None, fortran=False
):
    if logger == None:
        logger = Logger(None)
    ...
    if not fortran:
        ...
        while i <= (len(x) - minRep):
            ...
            if x[i] == x[i + minRep]:
                if np.all(
                    [j == x[i] for j in x[i : i + (minRep + 1)]]
                ):  # start and endtime is the same
            ...
            if x[i] == x[i + minRep] or abs(x[i] - x[i + minRep]) <= bound:
                if np.all([abs(j - x[i]) <= bound for j in x[i : i + (minRep + 1)]]):
                    while np.all(
                        [abs(j - x[i]) < bound for j in x[i : i + (count)]]
                    ) and i + count < len(x):
                        count += 1
