...
# Code base originally written by Tyler Ganter from Sandia National Laboratories
# Augmented for use within Pycheron by Pycheron team
...
class LogComressor(object):
    ...
    def transform(self, features):
        assert hasattr(self, "_mins_and_maxs"), "LogCompressor object has " "not been fit yet!"
...
class CalibrationHelper(object):
    ...
    def update_state(self, trace):
        ...
        snclq = trace.get_id()
        if self.fc_snclq != snclq or abs(self.fc_endtime - trace.stats.starttime) > self.fc_time_eps:
            self.fc_snclq = snclq
            self.fc_starttime = -99
            self.fc_endtime = -99
            self._update_filters()
    ...
    def _config(self):
        ...
        assert self.minDetectionDuration > self.durEnvFilter, (
            "gap between start and end times for a detection must be " "greater than the envelope filter start-up"
        )
...
def unscreened_calibration(trace, metric_store=None, debug=False, **params):
    ...
    def get_candidate_indices(x):
        ...
        candidate_indices = [i for i in candidate_indices if i - 1 not in candidate_indices]
        ...
        if 0 in candidate_indices and (
            _chelper.FCySmooth.empty() or _chelper.FCySmooth.get(0)[-1] > _chelper.threshold
        ):
    ...
    for cur_detection in cal_detections:
        ...
        _chelper.filters["envelope"].zi = np.zeros(len(_chelper.filters["envelope"].B) - 1)
    ...
    for cur_detection in cal_detections:
        ...
        if "do_recalibrate_start" in cur_detection:
            ...
            if trace.stats.starttime < temp_starttime:
                start_index = int((temp_starttime - trace.stats.starttime) * _chelper.fs)
                ...
                try:
                    assert start_index >= 0 and start_index <= len(xtemp), "not enough cached data"
...
def calibration(trace, metric_store=None, debug=False, **params):
    ...
    if len(detections) != 0:
        ...
        dups = [idx for idx, item in enumerate(iden_detections) if item in iden_detections[:idx]]
