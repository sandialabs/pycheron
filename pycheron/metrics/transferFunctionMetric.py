...
def transferFunctionMetric(tr1, tr2, logger=None, database_config=None, evalresp=None):
    ...
    This function calculates the transfer function from trace data. Response
    information is then obtained from the evalresp web service"
    (https://cran.r-project.org/web/packages/IRISMustangMetrics/IRISMustangMetrics.pdf) [#]_
    ...
    .. note::
        ...
        other. If sampling rates differ, and one is a multiple of the other, the stream with the higher sampling rate
        will be decimated to match the lower sampling frequency. The output dictionary generated for these two-channel
        metrics will have a SNCL code of the form: Net.Sta.Loc1:Loc2.Chan.
    ...
    **Algorithm Steps**
    ...
    2) Calculate transfer function values:
         ...
         * dataGain <- Mod(Txy)
         * dataPhase <- Arg(Txy)
         * Calculate and save the avgDataGain (amplitude) and avgDataphase values for T=5-7s
    3) Calculate the corresponding response amplitude ratio and phase difference for x and y
         ...
         * Calculate and save the avgRespGain and avgRespPhase values for T = 5-7s
    4) Calculate metrics:
         * Calculate (data/resp) gain ratio for two saved gain averages:
           gain_ratio = avgDataGain/avgRespGain (reasonableness of cross-spectral amplitudes between tr1 and
           tr2)
         * Calculate (data-resp) phase difference from the two saved phase averages:
           phase_diff = avgDataPhase - avgRespPhase (reasonableness of cross-spectral phases between tr1 and
           tr2)
         * Calculate mean squared coherence of x and y:

           * Retrieve stored PSD for Pxx and Pyy
           * ms_coherence = abs(Pxy)^2 / (Pxx*Pyy) (mean square coherence between tr1 and tr2)
           * Save the average MSCoherence for T=5-7s"
             (Copied from notes from Mary Templeton email in transferFunctionMetric.R in Callahan, 2020 and
    ...
    * Whenever ms_coherence ~= 1.0, properly functioning seismometers should have:
      ...
      * gain ratio ~= 1.0
      * phase_diff < 10.0 (degrees)" (https://cran.r-project.org/web/packages/IRISMustangMetrics/IRISMustangMetrics.pdf)
    ...
    **Example**
    ...
    .. code-block:: python
        ...
        print TF
        >>> {'metric_name': 'transferFunction', 'start_time': "2011-05-01T01:00:00.019500Z",
             'snclq': 'PASC:CI:BHZ:00:BHZ:10', 'end_time': "2011-05-01T01:59:59.994500Z",
             'ms_coherence': 0.9999875790954715, 'gain_ratio': 0.9058981533571073,
    ...
    # First though let's define default span for crossSpectrum calculation (Daniell smoothing)
    spans = np.array([3, 5])
    ...
    # Checking sampling rates > 1, if not exit
    if tr1.stats.sampling_rate < 1:
        logger.warn("transferFunctionMetric(): {} has a sampling rate < 1".format(str(tr1.get_id())))
        return

    if tr2.stats.sampling_rate < 1:
        logger.warn("transferFunctionMetric(): {} has a sampling rate < 1".format((str(tr2.get_id()))))
        return

    # Check if sampling rates differ, make sampling rate minimum of both traces if differs
    ...
    if sr1 > sampling_rate:
        if sr1 % sampling_rate != 0:
            logger.warn(
                ...
                "each other {s1}:{s2}".format(s1=str(tr1.get_id()), s2=str(tr2.get_id()))
            )
            return
        increment = round(sr1 / sampling_rate)
        # Check if sample rates differ by more than a factor of 10, if so exit
        if increment > 10:
            logger.error(
                "transferFunctionMetric(): Sampling rates differ by more than a factor of "
                "10 for {s1}:{s2}".format(s1=str(tr1.get_id()), s2=str(tr2.get_id()))
            )
            return
    ...
    # Check for valid data everywhere after potential resampling. If find NaNs exit
    ...
    # http://www.iris.edu/manuals/SEED_appA.htm" (Callahan, 2020)

    # Get channel name
    channel = tr1.stats.channel
    # Create lo, hi frequencies based on channel band code
    ...
    # Set an alignment frequency to generate octaves from
    alignFreq = 0.1

    # Truncate to nearest power of 2 samples
    ...
    for i in range(0, 13):
        ...
        #   http://cran.r-project.org/web/packages/psd/vignettes/normalization.pdf" (Callahan, 2020)
    ...
    # Convert phase from radians to degrees for comparison with evalresp response.
    dataAmp = abs(pxy / pxx)
    dataPhase = phase * 180 / pi

    # Calculate average values for periods in the 5-7 second period range for amp, phase, and ms coherence.
    ...
    for i in range(len(period)):
        if 5 <= period[i] <= 7:
            indices.append(i)
    # Calculate average values for amp, phase, MS coherence in 5-7 period range
    avgDataAmp = np.mean(dataAmp[indices])
    avgDataPhase = np.mean(dataPhase[indices])
    avgCoherence = np.mean(coh[indices])

    # Grab out network, station, channel,location information from traces for evalresp call
    ...
    # TODO T: Make distinction between evalresp/iris client
    if evalresp:
        e_filtered_1 = evalresp[channel1]
        e_filtered_2 = evalresp[channel2]

        respfile1 = platform_file_path(filename=e_filtered_1[1], dirname=e_filtered_1[0])
        respfile2 = platform_file_path(filename=e_filtered_2[1], dirname=e_filtered_2[0])

        # User provided FAP
        if e_filtered_1[2].upper() == "FAP" and e_filtered_2[2].upper() == "FAP":
            iris1 = fap_reader(respfile1)
            iris2 = fap_reader(respfile2)

        # User provided EVRESP
        elif e_filtered_1[2].upper() == "EVRESP" and e_filtered_2[2].upper() == "EVRESP":
            iris1 = read_Evalresp(
                sfft=minfreq,
                efft=maxfreq,
                nfft=nfreq,
                filename=respfile1,
                date=UTCDateTime(start1),
                station=station1,
                channel=channel1,
                network=network1,
                locid=location1,
            ...
            raise ValueError(
                f"Expected EVRESP and FAP. Received e_filtered_1: {e_filtered_1[2]}, e_filtered_2: {e_filtered_2[2]}"
            )
        # calculate amplitude ratio and phase difference for tr1 and tr2. Also get period
        respAmp = iris2[1] / iris1[1]
        respPhase = iris1[2] - iris2[2]
        period = 1 / iris1[0]

    # If evalresp is None, call client.evalresp to get response from IRIS for both traces
    else:
        ...
        # Calculate the corresponding response amplitude ratio and phase difference for tr1 and tr2. Also get period
        respAmp = iris2[:, 1] / iris1[:, 1]
        respPhase = iris1[:, 2] - iris2[:, 2]
        period = 1 / iris1[:, 0]
    ...
    # Calculate average response amp and phase for 5-7s period range
