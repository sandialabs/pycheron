...
if platform.system() != "Windows":
    try:
        ...
        print("------------------------------------------------------")
        subprocess.call(
            ["f2py", "-c", "-m", "hampel", "--quiet", fpath + "/outliers/outliers.f"]
        )
        os.chdir(pwd)
        try:
            import pycheron.hampel as hampelF
...
def hampel(data, nwin, ind):
    ...
    **Example**
    ...
    # Constants
    L = 1.4826  # Mean Absolute Deviation
    ...
    if nwin % 2 == 0:
        ...
        # Calculate the median of these two values
        absMinusMedian = np.sort(absMinusMedian)

        medianAbsMinusMedian = (
            absMinusMedian[(nwin // 2) - 1] + absMinusMedian[nwin // 2]
        ) // 2
    else:
        x0 = tmp[nwin // 2]
        # Calculate the absolute difference between each point and the median
...
def hampelFilter(data, nwin, increment):
    ...
    The Hampel filter is a robust outlier detector using Median Absolute Deviation (MAD); vector of values returned
    ...
    **Example**
    ...
    # For valid region, calculate result
...
def findOutliers(
    ...
    :type increment: int
    :param fixedThreshold: (boolean) - True/False specifying whether outlier detection uses selectivity
    ...
    **Plotting**
    ...
    if fortran:
        ...
        # If 50% of values in window are same, then h blows up to Inf. In this case, set to nan.
        h[h == np.inf] = np.nan

    # If all nans, exit finding outliers
    if np.isnan(h).all():
        print(
            "roll hampel returns a vector with all NaNs; 50% of values in all windows identical"
        )
        return
    ...
    if maxH < threshold:
        ...
        if fixedThreshold is True:
            ...
            outinds = [
                inds for inds, value in enumerate(h) if value > maxH * selectivity
            ]
