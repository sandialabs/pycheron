...
from pycheron.util.logger import *
...
def stationNoiseModel(
    ...
    **Plotting**
    ...
    # If obspy object calculate psds and grab out station name
    ...
    # If st is a database object, then grab psd from db. Grab out station name.
        ...
        psds = create_psds_from_metric_table(
            database, network, station, session, location, logger
        )
    ...
    # add to output dictionary and plot if plot == True, plot the single profiles using orangered for the E component,
    ...
    # high/low noise profiles, add to output dictionary and plot if plot == True, plot the envelopes using orangered
    ...
    # fname
        ...
        if plot:
            ...
            p.semilogx(
                out["z_station_noiseModel_low"], "darkmagenta", label="Z Station"
            )
...
def create_psds_from_metric_table(
    database, network, station, session=None, location=None, logger=None
):
    tb = database.get_metric(
        "psdMetric",
        network=network,
...
def gen_stats_plot_vals(psds, type, logger=None, database=None):
    ...
    for i in range(len(stats)):
        ...
        if (
            np.isnan(stats[i]["percent_5"]).all() != True
            and np.isnan(stats[i]["percent_10"]).all() != True
            and np.isnan(stats[i]["percent_90"]).all() != True
            and np.isnan(stats[i]["percent_95"]).all() != True
        ):
