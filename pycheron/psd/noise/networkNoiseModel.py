    ...
    **Plotting**
    ...
    if isinstance(st, obspy.core.stream.Stream):
        ...
        for i in range(len(sta)):
            ...
            stN = getSta(st, sta[i])
            # get psds for subset stream object and calculate psd statistics
            ...
            stats = psdStatistics(
                psds, logger=logger, evalresp=evalresp, database=database
            )
    ...
    if plot:
        ...
        if len(out["zModel"][0]) > 0:
            plt.semilogx(
                out["zModel"][0], out["zModel"][1], "c", label="Z Network Model"
            )
...
def unique_stations_psds_stats(
    ...
    for i in range(len(sta)):
        ...
        stats = psdStatistics(
            chan_psds, evalresp=None, logger=logger, database=database
        )
        # get period where freq < 10Hz
        try:
