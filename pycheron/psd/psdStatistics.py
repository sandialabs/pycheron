...
def psdStatistics(psds, evalresp=None, logger=None, database=None):
    """
    The psdStatistics function calculates a variety of basic statistics associated with an incoming set of PSDs.
    ...
    **Example**
    ...
    .. code-block:: python
        ...
        # only one dict in the list, then there is only one trace in the stream. Data can be accessed using the following
        # dict keys
    ...
    if logger == None:
        logger = Logger(None)
    # Hack -- change later (maybe?) If receive empty psds exit out
    k = []
    try:
        len(psds[0])
        if len(psds[0][0]) == 0:
            logger.error("psdStatitics(): psd input is empty or zero")
            return k
    except TypeError:
        logger.error("psdStatitics(): psd input is empty or zero")
        return k
    ...
    # Get list of frequencies for associated PSD as well as noise matrices for each corrected PSD
    ...
    # ------------Calculate PSD Max, Min, Mean, Median------------------------------------------------
    ...
    for h in range(len(psds)):
        ...
        # initialize containers with NaNs for storage based on ncol value from noise matrix (ncol = # of freqs)
        ...
        # Loop through rows of columns and fill in one frequency at a time for PSD max, min, mean, and median
        for i in range(ncol):
        ...
        # check one noise PSD as should be the same length
        try:
        ...
        # Calculate percent of total values above/below
        pctAbove = 100 * (aboveCount / nrow)
        pctBelow = 100 * (belowCount / nrow)

        # ------------Calculate Mode------------------------------------------------

        # For mode, we need to convert to the discretized pdfMatrix that contains in each bin, the count of PSDs that
        # have that value. The mode is just the bin with the highest count.
        # Good Default dbBins: lo=-200, hi=-50, binsize=1
        ...
        try:
            ...
            if np.isnan(lo) or np.isinf(lo) == True:
                lo = -200
            if np.isnan(hi) or np.isinf(lo) == True:
                hi = -50
        ...
        # ------------Output--------------------------------------------------------
        # Stuff output into dictionary and append to data
        d = {
