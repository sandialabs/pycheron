...
def test_start_and_end_times_match(BHZ_channel_stream_and_psd_list):
    results = psdStatistics(BHZ_channel_stream_and_psd_list[1])
    assert (
        UTCDateTime(results[0]["start_time"])
        == BHZ_channel_stream_and_psd_list[0][0].stats["starttime"]
    )
    assert (
        UTCDateTime(results[0]["end_time"])
        == BHZ_channel_stream_and_psd_list[0][0].stats["endtime"]
    )
