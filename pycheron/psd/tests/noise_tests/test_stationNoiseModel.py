...
def test_stationNoiseModel_returns_empty_list_for_channels_not_in_trace(
    snm_results_all_types, type
):
...
def test_stationNoiseModel_returns_correct_vals_for_noise_envelope10_90(
    ...
    assert result["z_station_noiseModel_high"].tolist()[0] == pytest.approx(
        -117.490234553, 0.01
    )
    assert result["z_station_noiseModel_high"].tolist()[-1] == pytest.approx(
        -74.1643614277, 0.01
    )

    assert result["z_station_noiseModel_low"].tolist()[0] == pytest.approx(
        -154.315109347, 0.01
    )
    assert result["z_station_noiseModel_low"].tolist()[-1] == pytest.approx(
        -98.5034601199, 0.01
    )

    assert (
        len(result["z_station_noiseModel_low"])
        == 72
        == len(result["z_station_noiseModel_high"])
    )
...
def test_stationNoiseModel_returns_correct_vals_for_noise_single05(
    ...
    assert result["z_station_noiseModel"].tolist()[0] == pytest.approx(
        -158.821993257, 0.01
    )
    assert result["z_station_noiseModel"].tolist()[-1] == pytest.approx(
        -100.284149714, 0.01
    )
