...
def crossSpectrum(x, y, fs, spans=None, taper=0.1, pad=0, demean=False, detrend=True, logger=None):
    """

    Calculates cross-spectral values between two time series

    :param x: Time series trace object
    ...
    :param taper: Specifies the proportion (i.e., percentage) of data to taper. A split cosine bell taper is applied to
                  this proportion of the data at the beginning and end of the series. Percentage in decimal format
                  (i.e. .1 = 10%)
    :type taper: float
    ...
    **Example**
    ...
    .. code-block:: python
        ...
        freq, spec1, spec2, coh, phase, pxx, pyy, pxy = crossSpectrum(tr1,
                                                                      tr2,
                                                                      sampling_rate,
    ...
    **Plotting**
    ...
    .. rubric:: References (reference list from original R IRISSeismic code base (Callahan, 2020))
    ...
    if spans is None:
        logger.error("crossSpectrum(): Must specify spans in order for smoothed periodogram estimate to be calculated")
        return

    # Get kernel dimension for modified daniell calculation
    m = spans // 2

    # TODO: Handle Error if spans size is zero.
    # If span size not equal to zero, get the modified daniel kernel of m
    if spans.size != 0:
    ...
    # "Correct for tapering: Bloomfield (1976, p. 194). Total taper is taper * 2"
    # (https://cran.r-project.org/web/packages/IRISSeismic/IRISSeismic.pdf)
    u2 = 1 - (5 / 8) * taper * 2
