...
class MetricStore(object):
    ...
    def put(self, snclq, metric_name, metric_value, date, overwrite=False, reshape=False):
        """
    ...
    class _DayTracker(object):
        ...
        def add(self, snclq, metric_name, metric_value, date):
            ...
            if not self._is_same(snclq, metric_name) or (len(self.dates) > 0 and date - self.dates[-1] != 24 * 3600):
                """
                either:
            ...
            return (date + 24 * 3600).month != date.month and len(self.dates) >= date.day
        ...
        def get_year_month_data(self):
            ...
            data = [self.values[i] for i, date in enumerate(self.dates) if date.year == year and date.month == month]
    ...
    def _year_month_index(year_month):
        ...
        if hasattr(year_month, "year") and hasattr(year_month, "month"):
            ...
            assert len(year_month) == 2, (
                "invalid input year_month" "\nmust be datetime-like object or " "tuple containing (year, month)"
            )
                ...
                if (
                    ...
                    try:
                        ...
                        assert len(avg_data) == store[metric_name].shape[1], (
                            ...
                            "snclq/metric_name"
                            "\nLength in store: %d"
                            "\nLength of new:   %d" % (store[metric_name].shape[1], len(avg_data))
                        )
