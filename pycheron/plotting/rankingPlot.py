...
from pycheron.plotting.psdPlot import *
        ...
        if model == "nnm":
            ...
            if database is not None:
                ...
                tb = database.get_metric(
                    "networkNoiseModel", network=network, session=session
                )
            ...
            if database is not None:
                ...
                for i in range(len(stats)):
                    ...
                    if chan.endswith("N"):
                        ...
                        m_period = (
                            1 / stats[i]["noise_matrix_frequency"][np.where(period)]
                        )
    ...
    if model == "nnm":
        ...
        for i in range(len(stats["mode"])):
            ...
            for j in range(len(stats["mode"][i])):
                stats["noise_matrix_frequency"][i][j] = json.loads(
                    stats["noise_matrix_frequency"][i][j]
                )
        ...
        snclq = stats["snclq"][
            0
        ]  # need for color grad but only uses network, so does not matter which snclq we use
...
def calc_stats_from_psds_rankplot(
    ...
    period = 1 / freq
    period = period[np.where(period > 0.1)]
    stats = psdStatistics(psd_channels[0][0][0], evalresp=None, database=database)
...
def calc_power_period_rankplot(stats, period):
    ...
    for i in range(len(stats)):
        chan = getChannelName(stats[i]["snclq"])
        stats[i]["noise_matrix_frequency"] = np.array(
            stats[i]["noise_matrix_frequency"]
        )
        if chan.endswith("N"):
            sta_n.append(stats[i]["snclq"].split(".")[1])
            chan_n = chan
