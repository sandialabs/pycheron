...
@pytest.mark.parametrize(
    "chan",
    [
        {"channel_file": "AK_GHO_001", "channel_name": "BHN"},
        {"channel_file": "AK_GHO_002", "channel_name": "BHZ"},
        {"channel_file": "AK_GHO_003", "channel_name": "BHE"},
    ],
)
def test_st_rankPlot(f_name, model, chan):
    mseed = f"test_data/{chan['channel_file']}.mseed"
    st = obspy.read(mseed)
    ...
    if f_name is None:
        assert os.path.exists("f_name" + chan["channel_name"] + "_1_AK.png")
        assert os.path.exists("f_name" + chan["channel_name"] + "_6.5_AK.png")
        assert os.path.exists("f_name" + chan["channel_name"] + "_30_AK.png")
        assert os.path.exists("f_name" + chan["channel_name"] + "_100_AK.png")
        os.remove("f_name" + chan["channel_name"] + "_1_AK.png")
        os.remove("f_name" + chan["channel_name"] + "_6.5_AK.png")
        os.remove("f_name" + chan["channel_name"] + "_30_AK.png")
        os.remove("f_name" + chan["channel_name"] + "_100_AK.png")
    else:
        assert os.path.exists(f_name + chan["channel_name"] + "_1_AK.png")
        assert os.path.exists(f_name + chan["channel_name"] + "_6.5_AK.png")
        assert os.path.exists(f_name + chan["channel_name"] + "_30_AK.png")
...
def test_db_rankPlot(f_name, model, chan):
    ...
    rval = rankPlot(db, model=model, f_name=f_name, network="AK", station="GHO", channel=chan)
    assert rval is None
...
@pytest.mark.parametrize(
    "chan", [{"channel_file": "AK_GHO_003", "channel_name": "BHE"}]
)
