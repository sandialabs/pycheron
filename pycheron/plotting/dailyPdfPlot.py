...
def dailyPdfplots(
    ...
    """
    Plots a color grid of the daily PDF Mode differences from noise model (nlnm or gsn)
    ...
    **Example**
    ...
    if isinstance(st, Database):
        ...
        if model == "gsn":
            ...
            model_gsn = pd.read_table(
                gsn, names=["period", "z_power", "h_power"], delimiter=r"\s+"
            )
            ...
            for stream in newStreams:
                ...
                df = pd.DataFrame()
                date_index = []
                # loop through traces in NEW streamsta
                for j in range(len(stream)):
                    ...
                    df2 = pd.DataFrame(
                        data=np.asarray(diff).reshape(1, len(diff)), columns=per
                    )
                    df = df.append(df2, ignore_index=True)

                    df_line2 = pd.DataFrame(
                        data=pmode.reshape(1, len(pmode)), columns=per
                    )
                ...
                update_line = _plot_colorline(
                    df_line, snclq, per_arr, f_name=f_name_line
                )
                ...
                try:
                    ...
                    max_period = min(
                        list(range(len(cols_f))), key=lambda i: abs(cols_f[i] - 10)
                    )
                    min_period = min(
                        list(range(len(cols_f))), key=lambda i: abs(cols_f[i] - 1)
                    )

                    # Find where values are below threshold, indicating a potentially dead channel
                    micro_thresh = np.where(
                        df.iloc[0][max_period:min_period] <= micro_threshold
                    )
...
def get_pdf_plot_data(
    ...
    if model == "nlnm":
        ...
        if tb.empty:
            logger.error(
                "dailyPDFPlot: Error no PSDS found in DB for Network: {n}, Station: {s}, Channel: {c}, Session: {se}".format(
        ...
        for i in range(len(tb.channel.unique())):
            ...
            for j in range(len(chan_psds)):
                ...
                df_line2 = pd.DataFrame(
                    data=pmode.reshape(1, len(pmode)), columns=period
                )
                df_line = df_line.append(df_line2, ignore_index=True)
...
def _pdf_plots_from_database(
    ...
    if model == "network":
        ...
        if net_dict is not None:
            ...
            # Comments inside of networkNoiseModel ask that the network MUST be defined if passing a database instead of stream
            # If this is not include, operations against different Networks will try to be performed/calculations will fail
...
def plot_grid(grd_data, snclq, plot, f_name=None, channel=None, rank=None):
    """
    Plots colorgrid data into Colorgrid (power mode or station ranking)
    ...
    if f_name != None and plot == "dailyPDF":
        name = f_name + "_" + snclq_id[3] + ".png"
        plt.savefig(name, dpi=dpi)
        d = {"pdf_grid_image_path": name}
        return d

    elif f_name != None and plot == "stationRanking":
...
def _plot_colorline(grd_data, snclq, per_arr=None, f_name=None):
    ...
    snclq_id = snclq.split(".")
    plt.title(
        "Daily PDF Mode Power Timelines: "
        + snclq_id[0]
        + " "
        + snclq_id[1]
        + " "
