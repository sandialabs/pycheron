...
def get_m_period(net_model):
    ...
    # if len EN net model > 0, grab out EN model period
    if len(net_model["enModel"][0]) > 0:
        m_period = net_model["enModel"][0]
    # elif len Z net model > 0, grab out Z model period
    elif len(net_model["zModel"][0]) > 0:
        m_period = net_model["zModel"][0]
    # Otherwise complain that can't assign model period
    else:
