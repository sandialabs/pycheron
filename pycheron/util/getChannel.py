...
def getChannelName(snclq):
    ...
    :rtype: str
    """
    # Get the channel name from a snclq object
...
def filterChannel(st):
    ...
    stN = st.copy()
    # Loop through stream object
    for i in range(len(st)):
        # For every trace, get the snclq information and the channel name. Not sure need to use getChannelName, can use
        # tr.stats.channel, so that might not really be that useful of a function
        tr = st[i]
        snclq = tr.get_id()
        chan = getChannelName(snclq)
        # If the channel is in the soh channel list, remove the trace from the stream object
        if chan in soh_chan:
            stN.remove(tr)
    return stN
